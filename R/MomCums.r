...
#'
#' @param mu_x the n-vector of cumulants starting from the fist - the mean
    ...
    if ( n>=3 ) {
      for(r in 2:(n-1)){
        ...
        for(m in 1:dim(el_jsr)[1]){
          ...
          Kr_m = KronProd(Kr_1)
          L_el = t(Commutator_Moment_L(el, m, r, d))
    ...
    if ( n>=3 ) {
      for(r in 2:(n-1)){
        ...
        for(m in 1:dim(el_jsr)[1]){### correggi dim ->ok!
          ...
          Kr_m = KronProd(Kr_1)
          L_el = t(Commutator_Moment_L(el, m, r, d)) ## check order r,m
          # Nota: probably as.matrix non fa quello che serve per cell2mat in matlab
          Prod_M_k[,m] = L_el%*%as.matrix(Kr_m)
        }
